...
def main():
    ...
    n_episodes = 100  # для 2м2м хорошо 150 количество эпизодов lapan = 20

    alpha = 0.9  # learning rate sayon - 0.5 больш - 0.9 Lapan = 0.2
    gamma = 0.5  # discount factor sayon - 0.9 больш - 0.5 lapan = 0.9
            ...
            for agent_id in range(n_agents):
                ...
                Q_table[agent_id, stateFoxint, action] = Q_table[agent_id, stateFoxint, action] + alpha * \
                                                         (reward + gamma * np.max(Q_table[agent_id, stateFox_next, :]) -
                                                          Q_table[agent_id, stateFoxint, action])
